...
func NewColorMarker(x, y, w, h int) *ColorMarker {
	m := &ColorMarker{}
	m.ControlBase = parts.NewControlBase(m, x, y, w, h)
	m.TextDrawable = parts.NewTextDrawable(m, "◀", 16, parts.AlignCenter, parts.AlignCenter, 0, 0, color.RGBA{0xff, 0xff, 0x00, 0xff}, false)
	return m
}
...
func NewColorPanel(x, y, w, h int) *ColorPanel {
	cp := &ColorPanel{}
	cp.ControlBase = parts.NewControlBase(cp, x, y, w, h)
	cp.Grouping = parts.NewGrouping(cp.ControlBase)
