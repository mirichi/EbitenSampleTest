...
func NewTitleBar(x, y, h int, text string) *TitleBar {
	t := &TitleBar{}
	t.InitTitleBar(x, y, h, text)
	return t
}

func (t *TitleBar) InitTitleBar(x, y, h int, text string) {
...
// ウィンドウのドラッグ移動に対応する
func NewClientArea(x, y int) *ClientArea {
	c := &ClientArea{}
	c.InitClientArea(x, y)
	return c
}

func (c *ClientArea) InitClientArea(x, y int) {
